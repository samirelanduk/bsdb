...
try:
    ...
    stage_interaction_ids = set([pdbmap[1] for pdbmap in interaction_pdb_maps])
    print("There are %i interactions with maps in the stage database." % (
     len(stage_interaction_ids)
    ))

    new_interaction_pdb_maps = [
     i for i in interaction_pdb_maps if i[1] not in live_interaction_ids
    ]
    new_stage_interaction_ids = set([pdbmap[1] for pdbmap in new_interaction_pdb_maps])
